...
class LogisticRegression(BaseModel):
    ...
    def _compute_cost(self, X: np.ndarray, y: np.ndarray) -> float:
        ...
        If class weights are provided, cost is weighted accordingly.
        """
        m = X.shape[0]
        if self.n_classes_ == 2:
            ...
            if self.config.class_weight is not None:
                # Separate cost for positive and negative samples using weights
                pos_idx = (y == 1)
                neg_idx = (y == 0)
    ...
    def _gradient_descent(self, X: np.ndarray, y: np.ndarray) -> None:
        """
        Optimize weights using gradient descent. Applies cost-sensitive weighting if provided.
        """
        m = X.shape[0]
        prev_cost = float('inf')
        ...
        for iteration in range(self.config.max_iter):
            if self.n_classes_ == 2:
                ...
                if self.config.class_weight is not None:
                    # Apply weights: multiply error element-wise based on true class
